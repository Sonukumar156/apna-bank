...
export default function Dashboard({ user, onLogout }) {
    ...
    const [showChangePassword, setShowChangePassword] = useState(!isAdmin && user.isFirstLogin === true)
    ...
    const navItems = isAdmin
        ? [
            ...
            { id: 'Member List', label: t('dashboard.members') },
            { id: 'Monthly Collection', label: t('dashboard.collection') },
            { id: 'Loan Management', label: t('dashboard.loans') },
            { id: 'Loan Requests', label: t('dashboard.loanRequests') },
            { id: 'Payment History', label: t('dashboard.history') },
            { id: 'Bonus Distribution', label: t('dashboard.bonus') }
        ]
        : [
            { id: 'My Profile', label: t('dashboard.myProfile') },
            { id: 'My Financials', label: t('dashboard.myFinancials') },
            { id: 'Loan Request', label: t('dashboard.loanRequests') },
            { id: 'Payment History', label: t('dashboard.history') }
        ]
    ...
    return (
        <div className="h-screen w-full bg-[#f8fafc] flex font-['Inter',_sans-serif] overflow-hidden text-slate-900 relative">
                ...
                <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
                    {navItems.map((item) => (
                        <button
                            ...
                            className={`w-full text-left px-5 py-3.5 rounded-xl lg:rounded-2xl text-[15px] lg:text-[16px] font-bold transition-all duration-200 flex items-center gap-4 ${activeTab === item.id
                                ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20'
                                : 'text-slate-500 hover:text-white hover:bg-white/5'
                                }`}
                        >
                            <div className={`w-2 h-2 rounded-full ${activeTab === item.id ? 'bg-white' : 'bg-slate-800'}`}></div>
                            {item.label}
                        </button>
                    ))}
                </nav>
            ...
            <main className="flex-1 flex flex-col overflow-hidden bg-white">
                <header className="px-4 md:px-8 lg:px-12 py-3 md:py-6 lg:py-8 flex justify-between items-center bg-white border-b border-slate-100 flex-shrink-0">
                    <div className="flex items-center gap-2 md:gap-4">
                        <button
                            className="lg:hidden p-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-600 active:scale-95 transition-all"
                            onClick={() => setIsSidebarOpen(true)}
                        >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 6h16M4 12h16M4 18h16" /></svg>
                        </button>
                        <div>
                            <h2 className="text-base md:text-xl lg:text-3xl font-bold text-slate-900 tracking-tight leading-none truncate max-w-[150px] md:max-w-none">{activeTab}</h2>
                            <p className="hidden md:block text-[10px] lg:text-xs font-bold text-slate-400 uppercase tracking-widest mt-1.5">System Active</p>
                        </div>
                    </div>
                    <div
                        className="flex items-center gap-2 lg:gap-6 cursor-pointer hover:bg-slate-50 p-1 lg:p-3 rounded-xl lg:rounded-2xl transition-all group"
                        onClick={() => setShowProfile(true)}
                    >
                        <div className="text-right hidden sm:block">
                            <p className="text-xs lg:text-base font-bold text-slate-900 group-hover:text-blue-600 transition-colors uppercase tracking-tight">{currentUser.name}</p>
                            <p className="text-[9px] lg:text-[10px] font-black text-blue-600 uppercase tracking-widest">{currentUser.role === 'admin' ? t('dashboard.roleAdmin') : t('dashboard.roleUser')}</p>
                        </div>
                        <div className="w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 bg-slate-900 text-white rounded-lg lg:rounded-2xl flex items-center justify-center font-bold text-xs md:text-sm lg:text-xl shadow-xl group-hover:scale-105 transition-all">
                            {currentUser?.name?.charAt(0) || 'U'}
                        </div>
                    </div>
                </header>

                <div className="flex-1 overflow-y-auto px-4 lg:px-10 py-5 lg:py-8 custom-scrollbar">
                    {isAdmin ? (
                        <div className="animate-in fade-in duration-500">
                                ...
                                <div className="space-y-6">
                                    {isAddingMember ? (
                                        <div className="animate-in slide-in-from-right-4 duration-500">
                                            ...
                                            <Register
                                                isAdminMode={true}
                                                onBackToLogin={() => setIsAddingMember(false)}
                                                onRegisterSuccess={() => {
                                                    refreshData()
                                        ...
                                        <>
                                            <div className="bg-slate-50 p-6 md:p-8 rounded-[32px] border border-slate-200/50 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
                                                <div>
                                                    <h3 className="text-lg md:text-xl font-bold text-slate-800">Member List</h3>
                                                    <p className="text-xs md:text-sm font-medium text-slate-500">Registered members in the committee.</p>
                                                </div>
                                                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full md:w-auto">
                                                    <div className="px-6 md:px-8 py-3 md:py-4 bg-white rounded-2xl border border-slate-200 text-center shadow-sm">
                                                        <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-1">Total Members</p>
                                                        <p className="text-xl md:text-2xl font-medium text-slate-900">{members.length}</p>
                                                    </div>
                                                    <button
                                                        onClick={() => setIsAddingMember(true)}
                                                        className="bg-slate-900 text-white px-6 py-4 rounded-2xl font-bold text-sm uppercase tracking-widest hover:bg-blue-600 transition-all shadow-xl active:scale-95"
                                                    >
                                                        + Add New Member
                                                    </button>
                                                </div>
                                                    ...
                                                    <thead className="bg-slate-50 border-b border-slate-200">
                                                        <tr>
                                                            {['Name', 'Mobile & Address', 'Monthly Plan', 'Duration', 'Join Date', 'Actions'].map(h => (
                                                                <th key={h} className="px-8 py-5 text-xs font-medium uppercase text-slate-500 tracking-widest">{h}</th>
                                                            ))}
                                                        </tr>
                                                    </thead>
                                                        ...
                                                        {members.map((m, i) => (
                                                            <tr key={i} className="hover:bg-slate-50/50 transition-colors group">
                                                                <td className="px-8 py-6">
                                                                    <p className="font-medium text-slate-800 text-base">{m.name}</p>
                                                                    <p className="text-[10px] font-medium text-blue-600 uppercase tracking-wide mt-1">ID: {m.regNo}</p>
                                                                    <p className="text-xs text-slate-400 mt-1">{m.email}</p>
                                                                </td>
                                                                <td className="px-8 py-6">
                                                                    <p className="text-base font-medium text-slate-800">{m.mobile}</p>
                                                                    <p className="text-xs font-medium text-slate-500 truncate max-w-[250px] mt-1">{m.address}</p>
                                                                </td>
                                                                <td className="px-8 py-6">
                                                                    <div className="flex items-center gap-2">
                                                                        <span className="w-1.5 h-1.5 rounded-full bg-blue-500"></span>
                                                                        <p className="text-base font-medium text-slate-900 tracking-tight">₹{(m.planAmount || 1000).toLocaleString()}</p>
                                                                    </div>
                                                                    <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wide mt-1">Monthly Contribution</p>
                                                                </td>
                                                                <td className="px-8 py-6">
                                                                    <div className="flex items-center gap-2">
                                                                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                                                        <p className="text-base font-medium text-slate-900 tracking-tight">{m.planDuration || '1'} Year(s)</p>
                                                                    </div>
                                                                    <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wide mt-1">Commitment</p>
                                                                </td>
                                                                <td className="px-8 py-6">
                                                                ...
                                                                <td className="px-8 py-6">
                                                                    <div className="flex gap-2">
                                                                        <button
                                                                            ...
                                                                            className="px-4 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-medium uppercase tracking-wide hover:bg-black transition-all"
                                                                        >
                                                                            View Profile
                                                                        </button>
                                                                        <button
                            ...
                            {activeTab === 'Monthly Collection' && (
                                <div className="space-y-10">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                                        <div className="bg-emerald-50 p-6 md:p-10 rounded-[32px] border border-emerald-100 flex items-center justify-between shadow-sm">
                                            <div>
                                                <p className="text-[10px] md:text-xs font-medium text-emerald-600 uppercase tracking-widest mb-2">Total Amount Paid</p>
                                                <h4 className="text-2xl md:text-5xl font-medium text-emerald-900 tracking-tight">₹{stats.totalCollection.toLocaleString()}</h4>
                                            </div>
                                            <div className="w-12 h-12 md:w-16 md:h-16 bg-white rounded-[16px] md:rounded-[20px] flex items-center justify-center text-emerald-600 shadow-sm border border-emerald-100">
                                        ...
                                        <div className="bg-rose-50 p-6 md:p-10 rounded-[32px] border border-rose-100 flex items-center justify-between shadow-sm">
                                            <div>
                                                <p className="text-[10px] md:text-xs font-medium text-rose-600 uppercase tracking-widest mb-2">Total Amount Due</p>
                                                <h4 className="text-2xl md:text-5xl font-medium text-rose-900 tracking-tight">₹{stats.pendingAmount.toLocaleString()}</h4>
                                            </div>
                                            <div className="w-12 h-12 md:w-16 md:h-16 bg-white rounded-[16px] md:rounded-[20px] flex items-center justify-center text-rose-600 shadow-sm border border-rose-100">
                                            ...
                                            {members.map((m, i) => (
                                                <div key={i} className="px-8 py-6 flex items-center justify-between hover:bg-slate-50 transition-colors">
                                                    <div className="flex items-center gap-5">
                                                        <div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center font-medium text-slate-400 text-lg">
                                                            {m?.name?.charAt(0) || '?'}
                                                        </div>
                                                        <div>
                                                            <p className="font-medium text-slate-900 text-xl leading-tight mb-1">{m.name}</p>
                                                            <span className={`text-xs font-medium uppercase tracking-wide px-3 py-1 rounded-lg border mt-1 inline-block ${m.financials?.collection?.status === 'paid' ? 'text-emerald-600 border-emerald-100 bg-emerald-50' : 'text-rose-600 border-rose-100 bg-rose-50'
                                                                }`}>
                                ...
                                <div className="space-y-6 lg:space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-slate-900 px-6 lg:px-10 py-6 lg:py-8 rounded-3xl text-white shadow-2xl relative overflow-hidden">
                                        <div className="relative z-10">
                                            <h3 className="text-xl lg:text-3xl font-bold tracking-tight">Loan Management</h3>
                                            <p className="text-slate-400 font-medium text-[10px] lg:text-sm mt-1 uppercase tracking-wide leading-none">Manage member loans</p>
                                        </div>
                                        <button
                                            onClick={() => { setSelectedMemberForLoan(null); setLoanError(''); setShowLoanModal(true); }}
                                            className="relative z-10 w-full sm:w-auto bg-blue-600 px-8 py-3 lg:py-4 rounded-xl lg:rounded-2xl font-medium text-xs lg:text-sm uppercase tracking-wide hover:bg-blue-500 hover:scale-105 active:scale-95 transition-all shadow-xl shadow-blue-600/20"
                                        >
                                            + New Loan
                                        </button>
                                    </div>
                                    ...
                                    <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
                                                ...
                                                <thead className="bg-slate-50 border-b border-slate-200">
                                                    <tr>
                                                        {['Member Name', 'Loan Details', 'Current Balance', 'Status'].map(h => (
                                                            <th key={h} className="px-6 lg:px-10 py-5 text-[9px] lg:text-[10px] font-medium uppercase text-slate-500 tracking-wide">{h}</th>
                                                        ))}
                                                    </tr>
                                                </thead>
                                                <tbody className="divide-y divide-slate-100">
                                                    {members.filter(m => m.financials?.loan?.active).map((m, i) => (
                                                        <tr key={i} className="hover:bg-slate-50/50 transition-colors group">
                                                            <td className="px-6 lg:px-10 py-5 lg:py-6">
                                                                <p className="font-medium text-slate-900 text-lg lg:text-xl leading-none mb-1">{m.name}</p>
                                                                <p className="text-[9px] lg:text-[10px] font-medium text-slate-400 uppercase tracking-wide">{m.financials.loan.loanId}</p>
                                                            </td>
                                                            <td className="px-6 lg:px-10 py-5 lg:py-6">
                                                                <p className="text-base lg:text-lg font-medium text-slate-900">₹{parseFloat(m.financials.loan.amount).toLocaleString()}</p>
                                                                <p className="text-[9px] lg:text-[10px] font-medium text-blue-600 uppercase mt-1 tracking-wide">At {m.financials?.loan?.interestRate || 0}% Interest</p>
                                                            </td>
                                                            <td className="px-6 lg:px-10 py-5 lg:py-6">
                                                                <p className="text-lg lg:text-2xl font-medium text-rose-600 tracking-tight">₹{parseFloat(m.financials.loan.remaining).toLocaleString()}</p>
                                                            </td>
                                                            <td className="px-6 lg:px-10 py-5 lg:py-6">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-medium uppercase tracking-wide border transition-all ${m.financials?.loan?.status === 'overdue' ? 'bg-rose-50 text-rose-600 border-rose-100' :
                                        ...
                                        {members.filter(m => m.financials?.loan?.active).length === 0 && (
                                            <div className="p-16 lg:p-24 text-center">
                                                <p className="text-[10px] lg:text-xs font-medium text-slate-300 uppercase tracking-wide">No active loans found</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            {activeTab === 'Loan Requests' && (
                                <LoanRequestManagement user={currentUser} isAdmin={true} onRefresh={refreshData} />
                            )}
                        </div>
                    ) : (
                        <div className="animate-in fade-in duration-500">
                            ...
                            {activeTab === 'Loan Request' && <LoanRequestManagement user={currentUser} isAdmin={false} onRefresh={refreshData} />}
